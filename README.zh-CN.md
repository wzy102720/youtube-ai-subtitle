...
一个油猴（Tampermonkey）脚本，**拦截 YouTube 自己的字幕请求**，预先批量翻译到你选的目标语言（默认中文），按时间戳叠加在视频上，显示成"目标语言在上、原文在下"的双语字幕。
...
- **本地缓存**：每个视频按目标语言各缓存一份，下次直接读缓存，秒出字幕
- **零中间商**：脚本和 API 直连，不经任何中间服务器
- **中英双语 UI**（v0.9.2+）：菜单和调试条文案首次启动时按浏览器语言自动选（`zh-*` → 中文，其他 → 英文）；油猴菜单里有"语言 / Language"项可手动切换
- **任意源语言 → 任意目标语言**（v0.10.0+）：源语言从 YouTube 字幕元数据自动取（英、西、日、韩…都支持）；目标语言通过菜单选（`zh-CN`、`en`、`ja`、`ko`、`es`、`fr`、`de`、`ru`、`pt`、`it`、`ar`、`hi`、`vi`、`th`、`id` 等任意 ISO 代码）。源语言和目标相同时自动跳过翻译。
...
菜单标签会跟着当前 UI 语言走。下面是中文 UI 下看到的标签，英文 UI 见 [README.md](README.md)。

| 菜单（中文 UI） | 作用 |
| --- | --- |
| 语言 / Language: 中文 | 在中文 / English 之间切换，刷新生效 |
| 设置 DeepSeek API Key | 弹窗填 / 改 API Key |
| 切换引擎 (当前: xxx) | 在 deepseek 和 google 间切换 |
| 目标语言 / Target: xxx | 弹窗输入目标语言代码（如 `zh-CN`、`en`、`ja`），刷新生效，持久化 |
...
- **0.10.0** 翻译管线可配置：
  - 目标语言现在用户可选——新增"目标语言 / Target"菜单项弹窗输入语言代码（中文 UI 默认 `zh-CN`，英文 UI 默认 `en`）
  - 源语言从 YouTube 字幕 URL 的 `?lang=` 参数自动识别，并在调试条里显示
  - DeepSeek 系统提示词和 Google `tl=` 参数都根据选定目标动态生成
  - 源语言和目标语言相同时直接跳过翻译，原文同时作为双语两条显示
  - cue 字段重命名 `en`/`zh` → `src`/`tgt`；CSS ID 同步重命名；字体栈扩成多语种（CJK + 拉丁 + 阿拉伯 + 印地 + 泰文）
  - 缓存 key 现在带目标语言后缀，换目标语言不会读到旧缓存
- **0.9.2** 加入中英双语 UI：菜单、弹窗、调试条文案首次启动按 `navigator.language` 自动选语言（`zh-*` → 中文，其他 → 英文）。新增"语言 / Language"菜单项可手动一键切换，选择通过 `GM_setValue` 持久化。脚本源码注释保留中文不动。
